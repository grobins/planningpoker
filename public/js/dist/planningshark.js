...
(function (planningShark) {
	...
	//card deck.
	planningShark.app.constant('deck', ['1', '2', '3', '5', '8', '13', '20', 'Inf', 'Coffee']);
...
(function (planningShark) {
	...
	planningShark.poker.createCtrl = function ($scope, $location, cookies) {
		...
		// };
		$scope.join = function () {
			var isScrumMaster = $scope.userName.toUpperCase() === "SCRUM MASTER"

			var roomName = 'analytics',
				path = '/room/' + roomName + '/' + $scope.userName + '/' + isScrumMaster;
			$location.path(path);
		};
...
(function (planningShark) {
	...
	planningShark.services.factory('room', function (socket, events, cookies) {
		...
		my.join = function (roomName, path, user) {
			...
			cookies.add(roomName, { name : roomName, path: path, user: user, joinDate : Date.now()}, {expires : 1});
